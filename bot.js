...
let welcomer = member.guild.channels.find('welcome', `روم الترحيب`)
...
.addField('» مضى على دخولك على الديسكورد ',`${moment(member.user.createdAt).format('D/M/YYYY h:mm a')} **\n**\`${moment(member.user.createdAt).fromNow()}\``,true)            
...
fs.readFile(`${w[Math.floor(Math.random() *w.length)]}`, function (err, Background) {
  ...
  let c = member.guild.channels.find('welcome', `روم الترحيب`)
