...
client.on('ready', () => client.user.setActivity(`?help , ?invite`, {type: "LISTENING"}))
...
client.on('message', message => {
        ...
        .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=542777444852039692&permissions=0&scope=bot`)
...
client.on('message',async msg => {
  var p = "?";
  if(msg.content.startsWith(p + "c-channel")) {
...
client.on('message', async msg => { // eslint disable line
    ...
    if (msg.content.startsWith(`${PREFIX}play`)) {
        ...
        } else if(msg.content.startsWith(`${PREFIX}wolfmohmed`)) {
        ...
        msg.author.send({embed: {
            ...
            fields: [{
                name: "Bot's commands:",
                value: `**${PREFIX} wolfmohmed** - This message!\n\
...
client.on('messageDelete', message => {
    ...
    var logChannel = message.guild.channels.find(c => c.name === 'night_log');
...
client.on('messageUpdate', (oldMessage, newMessage) => {
    ...
    var logChannel = oldMessage.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('roleCreate', role => {
    ...
    var logChannel = role.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('roleUpdate', (oldRole, newRole) => {
    ...
    var logChannel = oldRole.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('channelCreate', channel => {
    ...
    var logChannel = channel.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('channelUpdate', (oldChannel, newChannel) => {
    ...
    var logChannel = oldChannel.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('guildBanAdd', (guild, user) => {
    ...
    var logChannel = guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('guildMemberUpdate', (oldMember, newMember) => {
    var logChannel = oldMember.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('guildMemberAdd', member => {
  var logChannel = member.guild.channels.find(c => c.name === 'night_log');
  if(!logChannel) return;
...
client.on('voiceStateUpdate', (voiceOld, voiceNew) => {
    ...
    var logChannel = voiceOld.guild.channels.find(c => c.name === 'night_log');
    if(!logChannel) return;
...
client.on('guildCreate', guild => {
  ...
  client.channels.get("534467917589512203")
const embed = new Discord.RichEmbed()
   .setAuthor(`✅ | تم اضافه البوت في سيرفر جديد`)
   .setDescription(`**
...
Servers Counter : __${client.guilds.size}__**`)
         .setColor("#f3ae10")
         .addField("New Server")
         .setFooter('Night Bot' , client.user.avatarURL)
           client.channels.get("534467917589512203").send({embed}); //Sup
}
 
);

client.on('guildDelete', guild => {
  client.channels.get("534468021914304514")
const embed = new Discord.RichEmbed()
   .setAuthor(`❎ | تم طرد البوت من سيرفر`)
   .setDescription(`**
Server name: __${guild.name}__
Server id: __${guild.id}__
Server owner: __${guild.owner}__
...
Servers Counter : __${client.guilds.size}__**`)
         .setColor("#f3ae10")
         .setFooter('Night Bot' , client.user.avatarURL)
           client.channels.get("534468021914304514").send({embed});
}
...
embed.addField("🌪  الاسم", `**[ ${msg.author.username}#${msg.author.discriminator} ]**`, true)
   ...
   .addField('🎖  الرتبه', `**[ ${msg.member.roles.filter(r => r.name).size} ]**`, true)
